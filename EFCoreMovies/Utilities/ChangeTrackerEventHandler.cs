...
namespace EFCoreMovies.Utilities
{
    ...
    public class ChangeTrackerEventHandler: IChangeTrackerEventHandler
    {
        ...
        {
            var message = $"We processed {args.EntitiesSavedCount} changes.";
            logger.LogInformation(message);
        }
        ...
        {
            var message = $"Enitity: {args.Entry.Entity}, state: {args.Entry.State}";
            logger.LogInformation(message);
        }

        public void StateChangeHandler(object sender, EntityStateChangedEventArgs args)
        {
            var messagte = $"Entity: {args.Entry.Entity}, previous state: {args.OldState} - new state {args.NewState}";
            logger.LogInformation(messagte);
        }
    }
}
